...
class CondBlock(nn.Module):
    def __init__(self, ch_in, ch_out):
        ...
        self.pre = nn.Sequential(
            conv2d(ch_in, ch_out, 3, 1, 1, bias=False),
            Swish(),
        )
    ...
    def forward(self, feat, cond_feat):
        c = self.pre(cond_feat)
        return feat * self.alpha(c) + self.beta(c)
...
class SCBlock(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        assert in_ch == out_ch
        self.main =nn.Sequential(
            conv2d(in_ch, out_ch, 7, 1, 3, 1, in_ch),
            nn.Sigmoid()
        )
